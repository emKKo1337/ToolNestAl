...
function searchTools(raw: string): SearchResult[] {
  ...
  for (const tool of tools) {
    ...
    if (nameMatch || shortMatch || descMatch || kwMatch || catMatch) {
      // Rank: name match first, then keyword, then short desc, then category, then full desc
  ...
  // Sort: name > keyword > shortDescription > category > description
              ...
              <ul ref={listRef} role="listbox">
                {results.map((r, i) => (
                  <li
                    ...
                    onMouseDown={(e) => {
                      e.preventDefault(); // prevent input blur before navigate
                      navigate(r.href);
                    }}
